...
class Plugin
{
	...
	public function AddConversionTracking($order_id)
    {
		...
		if ( !empty( $order->get_coupon_codes() ) ) {
			$js_promotion_codes = json_encode($order->get_coupon_codes());
		}
		...
		echo "<script>
				jQuery(document).ready(function() {
					MI.send('conversion', {
					  ...
					  value: 			'".esc_attr( $order->get_total() - $order->get_shipping_total() )."',
					  currency: 		'".esc_attr( $order->get_currency() )."',
					  promotion_codes: 	'".wp_kses_post( $js_promotion_codes )."'
					});
				});
			</script>";
    }
    ...
    public function AddTracking($order_id)
    {
        ...
        echo '<img src="https://makeinfluence.com/p?' . esc_html(http_build_query([
    ...
    public function SendTracking($order_id)
    {
        ...
        $response = wp_remote_post('https://system.makeinfluence.com/track-conversion', [
            'body' => [
                ...
                'value' 			=> $order->get_total() - $order->get_shipping_total(),
